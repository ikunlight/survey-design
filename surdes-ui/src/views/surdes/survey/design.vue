    		...
    		<div class="de-left">
    			<el-tabs v-model="activeName" type="card" stretch @tab-click="handleClick">
						<el-tab-pane label="题型" name="first">
							<ul class="q-type-module" v-for="item in queModule" :key="item.title">
...
export default{
	...
	methods: {
      ...
      setTimeout(() => {
        this.$refs['question'][this.questionList.length - 1].isEdit = true
      }, 50)
			// this.updateIndex();
		},
		/** 保存问题 **/
		saveQuestion(question, index, func){
      ...
      console.log('####', this.questionList)
			this.questionList.forEach((q, i) => {
				...
				q.questionSort = i + 1;
        console.log('000022',q)
				this.queNoes.push({
					questionId: q.questionId,
					questionNo: q.questionNo,
					questionSort: q.questionSort
				})
        console.log('=====',this.queNoes)
			})
      ...
      console.log(11111, queNoes)
      if (queNoes != null && queNoes.length !== 0) {
        console.log('****')
